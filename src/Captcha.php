...
class Captcha implements CaptchaInterface
{
    ...
    public function __construct(Repository $config, Request $request)
    {
        ...
        $this->directory = dirname(__DIR__) . DIRECTORY_SEPARATOR . 'assets';
    }
    ...
    {
        if ($this->config->has($name = 'captcha.' . $config)) {
    ...
    public function create(string $config = null)
    {
        ...
        // 验证码使用随机字体
        $ttfPath = $this->directory . DIRECTORY_SEPARATOR . ($this->useZh ? 'zhttfs' : 'ttfs') . DIRECTORY_SEPARATOR;
        ...
        return tap(new Response($content, 200), function ($response) use($length) {
            $response->headers->set('Content-Type', 'image/png', true);
    ...
    {
        $path = $this->directory . DIRECTORY_SEPARATOR.'bgs'.DIRECTORY_SEPARATOR;
        $dir  = dir($path);
